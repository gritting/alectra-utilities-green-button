...
This guide explains how to run the Alectra Utilities Green Button exporter automatically using the AppDaemon add-on in Home Assistant OS.
...
```yaml
alias: Import Alectra Green Button Data
description: >-
  ...
  - choose:
      - conditions:
          - condition: trigger
            id:
              - download
          - alias: (debounce) if automation hasn't run in the last 2 minutes
            condition: template
            value_template: >-
              {{ now() -
              state_attr('automation.import_alectra_green_button_data',
              'last_triggered') | default( as_datetime('0'), true) >
              timedelta(minutes=2) }}
            enabled: true
        sequence:
      ...
      - conditions:
        ...
        sequence:
          ...
          - action: notify.mobile_app_your_phone_here
            metadata: {}
            data:
              title: New Energy Data Imported!
              message: New Energy Data {{ trigger.to_state.attributes.path }} imported!
              data:
